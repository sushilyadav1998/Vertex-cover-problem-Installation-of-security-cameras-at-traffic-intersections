...
class get_output_graph(object):
    ...
    #Get the list of intersection vertices
    def get_intersection_vertices(self):
        getverticeslist = self.get_vertices_map
        intersec_list = {}
        # print("GetVerticesList", getverticeslist)
        for vertices in getverticeslist:
            if getverticeslist[vertices] in self.get_intersection_list:
                intersec_key = vertices
                intersec_value = getverticeslist[vertices]
                intersec_list[vertices] = intersec_value
                
                # print(intersec_key, intersec_value)
                # print("Get Intersection points", intersec_list)
    ...
    while(True):
        ...
        if commands != 'g':
            ...
            if len(coordinates) < 2:
                print("Error: coordinates cannot be less than 2")
                continue
                ...
                #output()
                    ...
                    for num, value in enumerate(final_edges):
                        ...
                        # print ("<"+str(num[0])+","+str(num[1])+">"+",")
                            print("<{0},{1}>,".format(str(num[0]) , str(num[1])))
                        else:
                            print("<{0},{1}>".format(str(num[0]) , str(num[1])))
                    print ("}")
